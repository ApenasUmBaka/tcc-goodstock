...
function changePasswordVisibility(passwdInput: HTMLInputElement) {
  const setAttrTo =
    passwdInput.getAttribute("type") == "text" ? "password" : "text";
...
function setShowPasswdEvent() {
  ...
  showPasswdElements.forEach((elementName) => {
    ...
    const passwdElementName =
      elementName[4].toLowerCase() + elementName.slice(5);
    const passwdElement = document.getElementById(
      passwdElementName
    ) as HTMLInputElement;
...
const buttonsToChangeOrgModal = document.getElementsByClassName(
  "form__hvr-underline-from-left"
);
for (const button of buttonsToChangeOrgModal) {
  (button as HTMLAnchorElement).onclick =
    ModalController.toggleOrgInputs.bind(ModalController);
}
