...
function setEventsToInputs(
  elementName: string,
  validatorFunc: CallableFunction
) {
  ...
  elementField.onkeyup = () => {
    ...
    MessageErrorController.setMessage(validatorResult || "");
...
function confirmPasswordInput(
  passwdInputId: string,
  confirmPasswdInput: HTMLInputElement
): any {
  const passwdInput = document.getElementById(
    passwdInputId
  ) as HTMLInputElement;
  const confirmLabel = document.getElementById(
    `${confirmPasswdInput.id}Label`
  ) as HTMLInputElement;
...
if (document.getElementById("meta-microsoftAuth"))
  inputsToValidate.registerPasswd = () => {
    return;
  };
...
const registerConfirmPasswd = document.getElementById(
  "registerConfirmPasswd"
) as HTMLInputElement;
registerConfirmPasswd.onkeyup = confirmPasswordInput(
  "registerPasswd",
  registerConfirmPasswd
);

// Register organization confirm password.
const registerConfirmOrgPasswd = document.getElementById(
  "registerConfirmOrgPasswd"
) as HTMLInputElement;
registerConfirmOrgPasswd.onkeyup = confirmPasswordInput(
  "registerOrgPasswd",
  registerConfirmOrgPasswd
);
...
formsLogin.onclick = () => {
  ...
  const inputs = ["loginEmailLabel", "loginPasswdLabel"];
...
// Register customer with new organization.
const registerFormSubmit = document.getElementById(
  "submit-up-finish"
) as HTMLInputElement;
registerFormSubmit.onclick = () => {
  ...
  passwd.value = (
    document.getElementById("registerPasswd") as HTMLInputElement
  ).value;
...
loginFormSubmit.onclick = () => {
  const inputs = ["loginOrgIdLabel", "loginOrgPasswdLabel"];
