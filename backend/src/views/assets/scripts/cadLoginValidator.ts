...
class Validator {
  public static isNameValid(name: string): string | undefined {
    ...
    if (name.length < 3)
      return "Nome *Seu nome deve ter pelo menos 3 caracteres.";
    return;
  }
  ...
  public static isEmailValid(email: string): string | undefined {
    ...
    if (!regexEmail.test(email))
      return "Email *Por favor coloque um endereço de email válido.";
    return;
  }
...
function loginAccount(event: Event) {
  ...
  const loginPasswdLabel = document.getElementById(
    "loginPasswdLabel"
  ) as HTMLLabelElement;
...
const passwdField = document.getElementById(
  "registerPasswd"
) as HTMLInputElement;
passwdField.onkeyup = () => {
  const passwdLabel = document.getElementById(
    "registerPasswdLabel"
  ) as HTMLLabelElement;
  validateField(passwdField, "Senha", passwdLabel, Validator.isPasswdValid);
};

const confirmPasswdField = document.getElementById(
...
registerForm.onsubmit = () => {
  ...
  const orgIdLabel = document.getElementById(
    "registerOrgIdlabel"
  ) as HTMLLabelElement;

  const orgMasterPasswd = document.getElementById(
    "registerOrgPasswd"
  ) as HTMLInputElement;
  ...
  if (Validator.isPasswdValid(orgMasterPasswd.value)) {
    setLabelIncorrect(
      orgMasterPasswd,
      orgMasterPasswdLabel,
      "Master Password is not valid."
    );
    return false;
  }
...
loginForm.onsubmit = () => {
  ...
  passwd.value = (
    document.getElementById("loginPasswd") as HTMLInputElement
  ).value;
  return true;
};
